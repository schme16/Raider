	...
	*/
	function _createOptions(oOptions_) {
		...
		return {
			...
			delta : _oDelta  || null,
			position : oOptions_.position || {x:0,y:0},
	...
	function _onDevicemotion(event_) {
		...
		for (_sType in _oDatajQueryGestures) {
			...
			switch(_sType) {
				...
				case 'tilt':
					_aType =  [_sType];
					// calculate for webkit who returns the acceleration in each axe
					// left right
					_oPosition.x = Math.round(Math.asin( event_.accelerationIncludingGravity.x/9.81) * 180 / Math.PI);
					// front back
					_oPosition.y = Math.round(Math.asin(-1*event_.accelerationIncludingGravity.y/9.81) * 180 / Math.PI);

					_$element.triggerHandler(_sType, _createOptions ({type: _sType, description: _aType.join(':'), event:event_, position: _oPosition }) );
				break;
