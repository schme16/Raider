...
ig.Image = ig.Class.extend({
	...
	drawScale: function( x, y, scale, rotation ) {
		if( !this.loaded ) { return; }
		var rotate = rotation || 0
		var s = scale || 1
		var ctx = ig.system.context;
		ctx.save();	//save the context; there by preserving it.
			ctx.translate( x - ig.game.screen.x,  y - ig.game.screen.y) //shift to the relative center
			ctx.scale(s, s); //scale the image
			ctx.rotate(Math.deg(rotation)); //rotate it acordiingly
			this.draw(-(this.width/2),-(this.height/2))	; //then draw it with it's reference point at the center
		ctx.restore();
